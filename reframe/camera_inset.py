...
Ported from the OpenShorts reframe engine (camera_inset.py).
"""
...
def nearest_corner(box, frame_w, frame_h):
    ...
    cx = box[0] + box[2] / 2.0
    cy = box[1] + box[3] / 2.0
    return ("left" if cx < frame_w / 2 else "right", "top" if cy < frame_h / 2 else "bottom")
...
def is_cornered(box, frame_w, frame_h, margin=CORNER_MARGIN):
    ...
    near_edge = (
        x <= frame_w * margin
        or (x + w) >= frame_w * (1 - margin)
        or y <= frame_h * margin
        or (y + h) >= frame_h * (1 - margin)
    )
...
def detect(video_path, samples=10):
    ...
    from engine.reframe import screencast_layout
    ...
    close = (np.abs(centres_x - mid_x) <= tolerance) & (np.abs(centres_y - mid_y) <= tolerance)

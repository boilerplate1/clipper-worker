...
word timings in later needs no change here.

Ported from the OpenShorts reframe engine (punch_in.py).
"""
...
def emphasis_times(
    video_path, duration, window=BEAT_WINDOW, min_gap=MIN_GAP_SECONDS, prominence=BEAT_PROMINENCE
):
    ...
    """
    from engine.reframe import active_speaker

    envelope = active_speaker.audio_envelope(video_path, 0.0, duration, window_s=window)
    if not envelope:
        return []

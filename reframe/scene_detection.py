...
Ported from the OpenShorts reframe engine (scene_detection.py).
"""
...
def detect_scenes(video_path):
    ...
    if engine != "pyscenedetect":
        try:
            ...
            print(
                f"   ⚠️ TransNetV2 scene detection failed "
...
def _extract_frames_small(video_path):
    ...
    proc = subprocess.run(
        cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, check=True, timeout=900
    )
...
def _detect_transnetv2(video_path):
    ...
    scene_list = [(FrameTimecode(s, fps), FrameTimecode(e, fps)) for s, e in bounds]

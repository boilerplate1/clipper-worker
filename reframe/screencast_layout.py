...
Ported from the OpenShorts reframe engine (screencast_layout.py).
"""
...
def detect_faces_full_res(frame):
    ...
    with m.DETECT_LOCK:
        detector = m._get_face_detection()
        detections = detector.process(rgb) if detector else []
...
def detect_content_ranges(video_path, video_duration):
    ...
    for r in raw:
        try:
            ...
            end_s = min(float(video_duration), float(r.get("end", 0)))
        ...
        if end_s - s >= 0.5 and width >= MIN_WIDTH_FRACTION:
            ranges.append((s, end_s, str(r.get("what", ""))[:40], width))
    ranges.sort()

    if ranges:
        print(
            "   📊 "
            + ", ".join(f"{w}@{s:.0f}-{e:.0f}s ({frac:.0%} wide)" for s, e, w, frac in ranges[:5])
        )
    else:
...
def detect_screencast_scenes(video_path, scenes, strategies, ranges, samples=6):
    ...
    if not ENABLED or not ranges:
        return {}
    # Below the gate on purpose: this pulls cv2/mediapipe/torch, and the
    # disabled path (the default, and what CI exercises) must not pay it.
    import cv2
        ...
        for i, (start, end) in enumerate(scenes):
            ...
            for f_idx in np.linspace(s_f, last_f, samples):
                ...
                if centre is None:
                    ...
                    if person:
                        centre = (person[0] + person[2] / 2.0, person[1] + person[3] / 2.0)
                if centre:

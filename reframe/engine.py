...
Ported from the OpenShorts reframe engine (reframe_v2.py).
"""
...
from engine.reframe import active_speaker, camera_inset, punch_in, screencast_layout, split_layout
...
def _ffmpeg_binary():
    from engine.media.ffmpeg_utils import get_ffmpeg_executable

    return get_ffmpeg_executable()


def hwaccel_args():
    from engine.media.ffmpeg_utils import hwaccel_args as _ha
...
def delivery_size(orig_w, orig_h, aspect_ratio):
    ...
    if isinstance(aspect_ratio, str):
        if ":" in aspect_ratio:
            ...
            aspect_ratio = float(num) / float(den)
        else:
...
GENERAL_CONTENT_HEIGHT_RATIO = float(os.environ.get("GENERAL_CONTENT_HEIGHT_RATIO", "0.42"))
...
def general_filtergraph(out_w, out_h, content_h=None, top_pad=0, bottom_pad=0):
    ...
    fg_h += fg_h % 2
    pre = f"crop=w=iw:h=ih-{top_pad}-{bottom_pad}:x=0:y={top_pad}," if (top_pad or bottom_pad) else ""
    return (
        f"[0:v]{pre}split=2[bga][fga];"
...
def _analyze_trajectory(
    input_video, scenes_boundaries, scene_strategies, fps, orig_w, orig_h, cameraman, tracker
):
    ...
    from engine.reframe import analysis as m
    ...
    try:
        while True:
            ...
            if strategy in ("GENERAL", "SPLIT", "SCREENCAST", "WIDE", "INSET", "ALTERNATE"):
...
def render(input_video, final_output_video, aspect_ratio, content_ranges=None):
    ...
    if splits and active_speaker.ENABLED:
        for start_f in list(splits):
            ...
            if not active_speaker.is_conversation(verdicts):
                ...
                print(
                    f"   🔇 Scene {scene_idx}: one speaker holds the floor "
                ...
                alternates[start_f] = (active_speaker.hold(verdicts), splits.pop(start_f))
    if splits:
    ...
    if content_ranges:
        for scene_idx, (plan, centre) in screencast_layout.detect_screencast_scenes(
            input_video, scenes, strategies, content_ranges
        ).items():
    ...
    top_pad, bottom_pad = detect_letterbox(input_video)
    if top_pad or bottom_pad:
        print(f"   ✂️ Letterbox black bars detected: top={top_pad}px, bottom={bottom_pad}px — cropping")

    cameraman = m.SmoothedCameraman(
        out_w, out_h, orig_w, orig_h, aspect_ratio=aspect_ratio, top_pad=top_pad, bottom_pad=bottom_pad
    )
    tracker = m.SpeakerTracker(cooldown_frames=30)

    xs = _analyze_trajectory(
        input_video, scene_boundaries, strategies, fps, orig_w, orig_h, cameraman, tracker
    )
    if not xs:
    ...
    try:
        for idx, (start_f, end_f, strategy) in enumerate(ranges):
            ...
            if strategy == "INSET":
                ...
                graph = general_filtergraph(
                    out_w, out_h, full_width_content_height(orig_w, orig_h, out_w), top_pad=top_pad, bottom_pad=bottom_pad
                )
            elif strategy == "SPLIT":
                left, right = splits[start_f]
                graph = split_layout.split_filtergraph(orig_w, orig_h, out_w, out_h, left, right)
            elif strategy == "GENERAL":
                graph = general_filtergraph(out_w, out_h, top_pad=top_pad, bottom_pad=bottom_pad)
            else:
                ...
                if beats:
                    ...
                    first = boxes[0]
                    init = f"w={first[0]}:h={first[1]}:x={first[2]}:y={first[3] + top_pad}"
                else:
                    lines = dedupe_sendcmd_lines(seg_xs, fps)
                    init = f"w={crop_w}:h={crop_h}:x={seg_xs[0]}:y={top_pad}"
                with open(cmd_path, "w") as f:
                    f.write("\n".join(lines) + "\n")
                # The sendcmd f= value is parsed as a filter option, where a
                # colon ends the value and quotes do not protect it — an
                # absolute Windows path would break the graph. Run with the
                # workdir as cwd and hand over a bare filename instead.
                cmd_rel = f"cmd_{idx:03d}.txt"
                graph = (
                    f"[0:v]sendcmd=f='{cmd_rel}',crop@c={init},scale={out_w}:{out_h},setsar=1[v]"
                )
        ...
        # Total duration of the trimmed clip (sum of all segment durations).
        total_dur = sum((end_f - start_f) for start_f, end_f, _ in ranges) / fps

        # Concat video segments (stream copy) + audio straight from the clip.
        #
        # Each segment is video-only (-an) and its re-encoded duration can
        # differ slightly from the corresponding audio slice.  -shortest
        # stops the mux when the shortest stream ends (typically the video),
        # preventing the audio from extending past the video into a black
        # screen with audio only.  atrim is kept as a safety net for the
        # case where the audio stream has timestamp issues.
        _run(
            [
                _ffmpeg_binary(),
                "-y",
                ...
                "copy",
                "-af",
                f"aresample=async=1:first_pts=0,atrim=0:{total_dur:.4f},asetpts=PTS-STARTPTS",
                "-c:a",
                "aac",
                "-b:a",
                "192k",

...
Ported from the OpenShorts reframe engine (layout_picker.py).
"""
...
def apply(decision):
    ...
    from engine.reframe import active_speaker, screencast_layout, split_layout

    # sys.modules lookup first so tests can substitute stand-ins under the bare
    # names; the package attributes are the production fallback.
    modules = {
        "split_layout": sys.modules.get("split_layout") or split_layout,
        "screencast_layout": sys.modules.get("screencast_layout") or screencast_layout,
        "active_speaker": sys.modules.get("active_speaker") or active_speaker,
    }
...
def sample_frames(video_path, n=None, width=None):
    ...
    try:
        ...
        for i in range(n):
            ...
            h, w = frame.shape[:2]
            scaled = cv2.resize(
                frame, (width, max(2, int(h * width / w))), interpolation=cv2.INTER_AREA
            )
...
def pick_and_apply(video_path, video_duration):
    ...
    if SHADOW:
        ...
        print(
            f"[layout-shadow] decision={decision} "
